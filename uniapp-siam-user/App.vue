<script>
	...
	export default {
		...
		onLaunch: function() {
			...
			//   }
			// })
			this.globalData.selectCurrentInfo();
		...
		globalData: {
			...
			memberWeChatLogin(phoneNumber, openId, inviterId, userInfo) {
				https.request('/rest/member/weChat/login', {
					...
					if (result.success) {
						...
						toastService.showSuccess(result.data.message);
						this.isNewPeople = result.data.isNewPeople;
						this.getUserInfo();
			...
			checkIsAuth(authSetting, params) {
				toastService.showModal(
					...
					function confirm() {
						// #ifdef H5
						let url = '/subPackages_pages/internal/login/code/code';
						if (params) {
							if (params.inviterId) {
						...
						uni.getSetting({
							success: (res) => {
								...
								if (res.authSetting[authSetting]) {
									let url = '/subPackages_pages/internal/login/choose/choose';
									if (params) {
										if (params.inviterId) {
											url = url + (params.inviterId ? '?inviterId=' + params
												.inviterId : '');
										}
									}
									uni.navigateTo({
									...
									let url =
										'/subPackages_pages/internal/login/authorization/authorization';
									if (params) {
										if (params.inviterId) {
											url = url + (params.inviterId ? '?inviterId=' + params
												.inviterId : '');
			...
			getDeliveryAddressList(isDefault) {
				return new Promise((fulfil, reject) => {
					...
					if (result) {
						https.request('/rest/member/pointsMall/shoppingCart/list', {
							...
							if (result.success) {
								...
								this.setTabBarBadge(totalNum, 3);
							}
						});
						return;
					}
			...
			setTabBarBadge(num, index) {
				console.log(num, index)
				if (stringService.isEmpty(num)) {
					uni.setTabBarBadge({
						index: (index === undefined) ? 3 : index,
						text: String(num)
					});
					return;
				}
			...
			getShopList(location) {
				return new Promise((fulfil, reject) => {
					console.log(this.deliveryAndSelfTaking.location);
					if (this.deliveryAndSelfTaking.location) {
						var _this = this;
						https.request('/rest/shop/list', {
							pageNo: -1,
							pageSize: 1,
							position: location ? location : this.deliveryAndSelfTaking.location
						}).then((result) => {
							fulfil(result);
						});
					}
			...
			getShopInfo(shopId) {
				return new Promise((fulfil, reject) => {
					https.request('/rest/shop/detail', {
						id: shopId,
						position: this.deliveryAndSelfTaking.location
					}).then((result) => {
						if (result.success && result.data) {
							var selfOutActiveIndex = this.deliveryAndSelfTaking.selfOutActiveIndex;
							var selfOutActiveValue = this.deliveryAndSelfTaking.selfOutActiveValue;
							var selfOutItems = this.deliveryAndSelfTaking.selfOutItems;
							var shopInfo = result.data;
							
							//shopInfo.shop.checkoutMode=2;
							...
							this.deliveryAndSelfTaking.selfOutItems = selfOutItems;
							this.deliveryAndSelfTaking.selfOutActiveIndex = selfOutActiveIndex;
							this.deliveryAndSelfTaking.selfOutActiveValue = selfOutItems[
								selfOutActiveIndex].value;
							this.deliveryAndSelfTaking.showSelfOutItems = true;
							this.deliveryAndSelfTaking.shopInfo = shopInfo;
							fulfil(result.data);
						}
					});
				});
			},
			selectCurrentInfo: function() {
				var _this = this;
				return new Promise((fulfil, reject) => {
					https.request('/rest/setting/selectCurrent', {}).then((result) => {
						if (result.success) {
							//Integer shopManagementMode '门店经营模式 1=单门店 2=多门店'
							//Integer defaultShopId  '单门店时，默认接单的门店'
							//result.data.shopManagementMode = 2;
							// result.data.defaultShopId = 16;
							_this.deliveryAndSelfTaking.selectCurrent = result.data;
							fulfil(result.data)
						}
						if (!result.success) {
							toastService.showToast("门店基础数据未设置")
						}
					});
				});
			},
