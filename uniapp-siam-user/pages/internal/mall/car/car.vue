	...
	export default {
		...
		methods: {
			...
			goToPay() {
				...
				if (isTap) {
					uni.navigateTo({
						url: '/subPackages_pages/internal/mall/pay/pay?orderDetail=' + JSON.stringify(
							orderDetail) + '&payType=car'
					});
					return;
				}
				this.goToTap();
			...
			getShoppingCartList() {
				https.request('/rest/member/pointsMall/shoppingCart/list', {
					...
					if (result.success) {
						...
						totalNum = result.data.records.length;
			...
			updateNumber(id, number, type, callbak) {
				https.request('/rest/member/pointsMall/shoppingCart/updateNumber', {
					id: id,
					number: number,
					type: type
				}).then((result) => {
