	...
	<view class="page">
		...
		<view class="self_out_items">
			<view class="self_out_item" hover-class="hover-class-public" @tap="businessTap" data-index="1"
				v-if="selectCurrent.shopManagementMode==2">
			...
			<view class="self_out_item" hover-class="hover-class-public" @tap="businessTap" data-index="2">
...
<script>
	...
	export default {
		...
		methods: {
			...
			shoppingAddressTap() {
				uni.navigateTo({
					url: `/subPackages_pages/address/replace/replace?jump_page=index`
				});
			},
			...
			businessTap(e) {
				...
				app.globalData.selectCurrentInfo().then(result => {
					...
					if (index == 1) {
						...
						app.globalData.deliveryAndSelfTaking.selfOutActiveValue = index;
						uni.switchTab({
							url: `/pages/menu/index/index`
						});
					}
					if (index == 2) {
						_this.getDeliveryAddressList();
					}
				});
			},
			getDeliveryAddressList() {
				...
				authService.checkIsLogin().then((result) => {
					...
					if (result) {
						app.globalData.getDeliveryAddressList(false).then(records => {
							if (records.length == 0) {
								toastService.showModal('', '暂无配送地址',
									function() {
										_this.setData({
							...
							if (records.length > 0) {
								var shopAddress = records[0];
								var location = shopAddress.longitude + "," + shopAddress.latitude;
								app.globalData.getShopList(location).then(shop_result => {
									if (shop_result.data.records.length == 0) {
										toastService.showModal('', '该区域外送功能即将开通',
											function() {});
									};
									if (shop_result.data.records.length > 0) {
										var shopId = shop_result.data.records[0].id;
										app.globalData.getShopInfo(shopId).then(shopInfo => {
											if (shopInfo) {
												app.globalData.deliveryAndSelfTaking.selfOutActiveIndex = 2;
												app.globalData.deliveryAndSelfTaking.selfOutActiveValue = 2;
			...
			searchBusinessTap(e) {
				console.log(this);
				uni.navigateTo({
					url: '/subPackages_pages/menu/search/search?location=' + app.globalData.deliveryAndSelfTaking
						.location
				});
			},
			...
			commodityDetailTap(e) {
				uni.navigateTo({
					url: '/subPackages_pages/menu/detail/detail?id=' + e.currentTarget.dataset.id + '&shopId=' + e
						.currentTarget
						.dataset.shopid
				});
			},
