	...
	<view>
		...
		<view class="button-view">
			<button class="getphonenumber" type="primary" @getphonenumber="getPhoneNumber" open-type="getPhoneNumber"
				style="font-size: 30rpx">手机号快捷登录</button>
	...
	export default {
		...
		methods: {
			getPhoneNumber: function(e) {
				...
				//   return
				// }
				toastService.showToast('暂不支持手机号快捷登录，请选择验证码登录');
			},
			...
			wxLogin(iv, encryptedData, code) {
				...
				https.request('/rest/wxLogin/getWxPhone', {
					...
					if (result.success) {
						toastService.showLoading('正在登录...', true);
						app.globalData.weChatLogin(result.data.phoneNumber, result.data.openid, _this.data
							.inviterId);
					}
				});
			},
			...
			verificationCodeTap() {
				...
				if (this.inviterId) {
					...
					uni.navigateTo({
						url: '/subPackages_pages/internal/login/code/code?inviterId=' + this.inviterId
					});
				} else {
					console.log('choose-log-in.js -> 邀请者为空');
					uni.navigateTo({
						url: '/subPackages_pages/internal/login/code/code?inviterId'
					});
				}
			}
		}
