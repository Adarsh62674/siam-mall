	...
	export default {
		...
		methods: {
			...
			toPay() {
				...
				uni.navigateTo({
					url: '/subPackages_pages/internal/mall/pay/pay?payType=' + this.chooseType + '&orderDetail=' + JSON.stringify(orderDetail)
				});
			},
			...
			businessTap(e) {
				uni.navigateTo({
					url: '/subPackages_pages/internal/mall/menu/index/index?id=' + e.currentTarget.dataset.id
				});
			},
			...
			bindShare(e) {
				authService.checkIsLogin().then((result) => {
					if (result) {
						uni.navigateTo({
							url: '/subPackages_pages/mine/share/index/index?inviterId=' + this.userInfo.id
						});
					} else {
						app.globalData.checkIsAuth('scope.userInfo');
					}
