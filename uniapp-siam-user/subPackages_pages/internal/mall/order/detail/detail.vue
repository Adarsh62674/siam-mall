	...
	export default {
		...
		methods: {
				...
				authService.checkIsLogin().then((result) => {
					if (result) {
						...
						app.globalData.getIsBusiness(startTime, endTime, isOperating).then((result) => {
							...
							console.log(this);
							uni.navigateTo({
								url: '/subPackages_pages/address/choose/choose?radioIndex=0&pageType=orderDetail&orderNo=' +
									this.order.orderNo +
									'&orderId=' +
									this.order.id +
									'&deliveryData=' +
			...
			applyRefund(e) {
				...
				uni.navigateTo({
					url: '/subPackages_pages/internal/mall/order/refund/apply/apply?orderId=' + this.order.id + '&type=' + e.currentTarget.dataset.type
				});
			},
			...
			bindRefundProcess() {
				uni.navigateTo({
					url: '/subPackages_pages/internal/mall/order/refund/progress/progress?orderId=' + this.order.id
				});
			},
			...
			selectLogistics(e) {
				...
				uni.navigateTo({
					url: '/subPackages_pages/internal/mall/order/logistics/logistics?logisticsJson=' + JSON.stringify(logisticsJson)
				});
			},
			...
			evaluateTip(e) {
				...
				uni.navigateTo({
					url: '/subPackages_pages/internal/mall/order/evaluate/evaluate?orderId=' + id + '&shopId=' + shopId
				});
			},
