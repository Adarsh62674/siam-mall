	...
	<view>
					...
					<view class="commdity-type-item" v-for="(item, key) in specList" :key="key">
						...
						<radio-group class="radio-group" @change="radioChange" :data-firstIndex="key">
							<label v-for="(item, index) in item" :key="index">
								...
								<view :class="'group-label theme-border ' +
		...
		<view class="shopping-cart-detail">
			<view class="content-fullReductionRuleName">
				{{
                    !shopInfo.isOutofDeliveryRange ? shopInfo.isOperatingOfShop && shopInfo.shop.isOperating
	...
	export default {
		...
		methods: {
			...
			bindMinus: function(e) {
				...
				if (number == 1) {
					...
					toastService.showModal(null, '确定不要这个了吗？', function confirm() {
						toastService.showLoading();
						that.shoppingCartUpdateNumber(cartId, 1, 0, function callback() {
							totalNum--;
							...
							that.getShoppingCartList(this.shopId, this.diningTable.tableId);
						});
					});
					return;
				}
			...
			insertShoppingCart(e) {
				...
				authService.checkIsLogin().then((result) => {
					...
					if (result) {
						...
						toastService.hideLoading();
						this.shoppingCartInsert(data, _this.diningTable.tableId);
						return;
					}
			...
			shoppingCartInsert(data, tableId) {
				...
				orderJs.shoppingCartInsert(data, tableId).then(result => {
					if (result.success) {
						...
						_this.getShoppingCartList(data.shopId, tableId);
					}
				});
			},
			goToPay() {
			...
			toPay() {
				...
				orderDetail.packingCharges = this.selfOutActiveValue != 0 ? packingCharges : 0;
				setTimeout(() => {
			...
			getShopInfo(shopId) {
				app.globalData.getShopInfo(shopId).then((result) => {
					if (result) {
						...
						if (this.diningTable.tableId) {
							this.getShoppingCartList(shopId, this.diningTable.tableId);
						} else {
							this.getFullReductionRule(result);
						}
					}
				});
			},
			...
			getFullReductionRule(shopInfo) {
				...
				this.fullReductionRuleName = fullReductionRuleName;
				this.fullPriceReductionIsHidden = fullPriceReductionIsHidden;
				this.isItNear = app.globalData.deliveryAndSelfTaking.isItNear;
				this.getShoppingCartList(shopId, this.diningTable.tableId);
			},
			getShoppingCartList(shopId, tableId) {
				var _this = this;
				orderJs.memberShoppingCartList(shopId, tableId).then((car_result) => {
					if (car_result.records.length > 0) {
						var packingCharges = 0;
						var totalNum = 0;
						var totalPrice = 0;
						var number = 0;
						...
						this.data.number = 0;
						car_result.records.forEach((result, index) => {
							let specList = '';
							for (var key in JSON.parse(result.specList)) {
								specList = (specList ? specList + '/' : specList) + JSON.parse(
									result
									.specList)[key];
							}
							result.restructure = specList;
							totalNum = totalNum + result.number;
							result.goodsPrices = utilHelper.toFixed(Number(result.goodsPrice) *
								result
								.number, 2);
							totalPrice += result.price * result.number; //初始化被选中的商品的总金额
							result.disable = result.goodsStatus == 1 || result.goodsStatus == 3 ||
								result
								.goodsStatus == 4 ? true : false;
							packingCharges =
								result.goodsStatus == 1 || result.goodsStatus == 3 || result
								.goodsStatus ==
								4 ?
								packingCharges :
						...
						setTimeout(() => {
							...
							_this.packingCharges = _this.selfOutActiveValue != 0 ? packingCharges : 0;
							var startDeliveryPrice = _this.shopInfo.shop.startDeliveryPrice;
							if (totalPrice + _this.packingCharges >= startDeliveryPrice) {
								isStartDeliveryPrice = true;
							}
							console.log('获取当前店铺的起送费====', startDeliveryPrice);
							var priceDifference = startDeliveryPrice - (totalPrice + _this.packingCharges);
							...
							_this.shoppingCartList = car_result.records;
							_this.totalPrice = utilHelper.toFixed(totalPrice + _this.packingCharges, 2);
							if (_this.selfOutActiveValue != 0) {
								_this.getShopCartFullReductionRule(shopId);
							}
						}, 500);
					}
				});
			...
			//获取满减规则
			getShopCartFullReductionRule(shopId) {
						...
						for (let key in specList) {
							...
							for (let keyof in specList[key]) {
								...
								if (specList[key][keyof].stock == 1 && isChecked) {
									...
									//选中的规格价钱在商品价钱的基础上累加
									price = price + specList[key][keyof].price;
									specListString = (specListString ? specListString + '/' :
											specListString) +
