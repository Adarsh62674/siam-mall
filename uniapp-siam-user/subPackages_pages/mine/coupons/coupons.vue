							...
							<view
								...
								<view class="bottom-view">
									<view class="usage-rule" @tap="onClick" :data-index="index">
										使用规则
										<van-icon name="arrow" v-if="item.couponsMemberRelationMap.isShow" />
										<van-icon name="arrow" v-if="!item.couponsMemberRelationMap.isShow" />
									</view>
									<view :class="
                                            'immediate-use ' +
			...
			<swiper-item class="swiper-items">
				<scroll-view @scrolltoupper="onPullDownRefresh" upper-threshold="-50"
					...
					<view class="coupins-items">
						<radio-group>
							<view
								:class="'coupins-item ' + (!item.couponsMemberRelationMap.isExpired && !item.couponsMemberRelationMap.isUsed && item.couponsMemberRelationMap.isRelation ? '' : 'isRelation')"
	...
	export default {
		...
		methods: {
			...
			onImmediatePointsUse(e) {
				...
				uni.switchTab({
					url: '/pages/internal/mall/index/index'
				});
			},
