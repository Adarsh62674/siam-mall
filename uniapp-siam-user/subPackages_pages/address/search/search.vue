	...
	export default {
		...
		methods: {
			bindInput: function(e) {
				...
				var address = keywords ? keywords : _this.region.join('');
				if (!address) {
					this.tips = [];
					return;
				}
			...
			bindSearch: function(e) {
				var searchKeywords = e.currentTarget.dataset.keywords;
				console.log(searchKeywords);
				if (!searchKeywords.location) {
					toastService.showToast("当前地址不可用");
					return
				}
				this.prevPage(searchKeywords);
			},
			...
			prevPage(data) {
				...
				let city;
				let regins;
				//解析点击选择的地址省市区，因选择地址district会出现省市区
				//定位和选择的数据不同步所以要操作以下代码
				if (data.district) {
					...
					regins = [district ? district[0] + '省' : city[0] + '市', city[0] + '市', city[1]];
				}
				console.log(regins);
			...
			getRegeoAddress() {
				...
				myAmapFun.getRegeo({
					success: function(data) {
						...
						for (var key in areaList) {
							...
							if (key == "city_list") {
								...
								for (var key_list in areaList[key]) {
									...
									//console.log(key_list, areaList[key][key_list], city.indexOf(areaList[key][key_list]));
									if (city == areaList[key][key_list] || city.indexOf(areaList[key][
											key_list
										]) != -1) {
										areaValue = key_list;
									}
						...
						city = city.length > 0 ? city : province.indexOf("市") ? province : '北京市';
				...
				uni.getSetting({
					success(res) {
						...
						if (res.authSetting['scope.userLocation']) {
							...
							uni.openSetting({
								success(openRes) {
									console.log("authSetting", openRes.authSetting);
									// res.authSetting = {
									//   "scope.userInfo": true,
