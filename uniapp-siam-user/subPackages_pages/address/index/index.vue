				...
				https.request('/rest/member/deliveryAddress/list', {
					...
					if (result.success) {
						result.data.records.forEach((item, index) => {
							var addressCity = item.province + item.city;
							if (item.province && item.province.indexOf("市") != -1) {
								addressCity = item.province;
							}
							item.addressCity = addressCity + item.area +" "+ item.street;
							console.log(item.addressCity)
