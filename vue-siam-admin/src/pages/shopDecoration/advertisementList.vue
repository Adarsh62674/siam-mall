			...
			<el-form :inline="true" :model="searchMsg">
				...
				<el-form-item label="轮播图类型" prop="type">
					<el-select v-model="searchMsg.type" clearable>
						...
						<el-option label="商城页面轮播图" :value="3"></el-option>
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" :cell-style="cellStyle" :header-cell-style="headerCellStyle">
			...
			<el-table-column prop="type" label="轮播图类型">
				<template scope="scope">
					...
					<span v-if="scope.row.type == 3">商城页面轮播图</span>
			...
			<el-form class="advertisementForm" :model="editForm" label-width="150px" style="width: 80%;" :rules="editFormRules" ref="editForm">
				<el-form-item label="轮播图类型" prop="type">
					<el-radio-group v-model="editForm.type" size="medium">
						...
						<el-radio-button label="商城页面轮播图（宽450px * 高225px）" value="3"></el-radio-button>
	...
	export default {
		...
		methods: {
			...
			formatType (row, column) { // 0=正常/启用  1=禁用
				return row.type == 1 ? '首页轮播图' : (row.type == 2 ? '菜单页轮播图' : (row.type == 3 ? '商城页面轮播图' : (row.type == 4 ? '分享页面生成美图' : '-')))
			},
			...
			handleEdit: function (index, row) { // 显示编辑界面
				...
				if(row){
					this.editForm = {
						id: row.id,
						type: (row.type == 1) ? '首页轮播图（宽450px * 高225px）' : ((row.type == 2) ? '菜单页轮播图（宽450px * 高185px）' : ((row.type == 3) ? '商城页面轮播图（宽450px * 高225px）' : ((row.type == 4) ? '分享页面生成美图（宽1080px * 高1854px）' : '-'))),
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						param.type = (param.type == '首页轮播图（宽450px * 高225px）') ? 1 : ((param.type == '菜单页轮播图（宽450px * 高185px）') ? 2 : ((param.type == '商城页面轮播图（宽450px * 高225px）') ? 3 : ((param.type == '分享页面生成美图（宽1080px * 高1854px）') ? 4 : '')));
