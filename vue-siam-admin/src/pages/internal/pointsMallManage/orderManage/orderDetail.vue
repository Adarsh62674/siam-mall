<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="searchMsg">
				<el-form-item>
					...
					<!-- <el-button type="primary" @click="printOrderDetail">打印</el-button> -->
		...
		<el-table :data="relatedList" highlight-current-row v-loading="relatedListLoading" style="width: 100%;" :cell-style="cellStyle" :header-cell-style="getRowClass">
			...
			<el-table-column prop="deliveryFee" label="运费" :formatter="addUnit"></el-table-column>
	...
	export default {
		...
		methods: {
			...
			speckText(str) {
				...
				audio.src = "https://siam-hangzhou.oss-cn-hangzhou.aliyuncs.com/data/file/order_tip.mp3";
				audio.play();                        
			},
			speckTextChangeToDelivery(str) {
				audio.src = "https://siam-hangzhou.oss-cn-hangzhou.aliyuncs.com/data/file/order_change_to_delivery_tip.mp3";
				audio.play();                        
			},              			
			testPlayAudio() {
				if(count%2 == 1){
			...
			printOrderDetail() {				
				...
				//获取订单信息
				let vue = this
				vue.$http.post(vue, '/rest/admin/pointsMall/order/selectById', {"id" : id},
					(vue, data) => {
						let order = data.data;

						let shoppingWayText = order.shoppingWay == 1 ? '自取' : '发货';
						if(order.isChangeToDelivery){
							shoppingWayText = '由自取订单改为发货';
						}
						...
						//获取订单明细信息
						vue.$http.post(vue, '/rest/admin/pointsMall/orderDetail/list', {"pageNo" : -1, "pageSize" : 20, "orderId" : id},
							(vue, data) => {
								...
								for(let i = 0; i < orderDetailList.length; i++){
									...
									let AData_label = '['+
											'{'+
												'"queueNumber": "'+ order.queueNo +'", '+
												'"shoppingWay": "'+ (order.shoppingWay == 1 ? '自取' : '发货') +'", '+
			...
			getRelatedList(id) { // 获取商品列表
				...
				vue.$http.post(vue, '/rest/admin/pointsMall/order/list', param,
					(vue, data) => {
			...
			getList(id) { // 获取商品列表
				...
				vue.$http.post(vue, '/rest/admin/pointsMall/orderDetail/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				vue.$http.delete(vue, '/rest/admin/pointsMall/orderDetail/delete', {"ids" : [id]},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){
							url = '/rest/admin/pointsMall/orderDetail/update';						
						}else{
							url = '/rest/admin/pointsMall/orderDetail/insert';	
