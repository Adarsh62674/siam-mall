            ...
            get() {
                return {
                    ...
                    init: function () {
                        ...
                        //改成12秒轮询一次
                        let timer = setInterval(this.autoPrintOrderInfo, 30*1000);
                    ...
                    autoPrintOrderInfo() { // 获取订单列表      
                        ...
                        $.ajax({
                            type: "post",
                            url: urlPrefix + '/rest/admin/order/waitPrintOrderList',
                            async: false,
                            data: {},
                            ...
                            success: function(data){
                                ...
                                if(orderList!=undefined && orderList.length>0){
                                    for(let i = 0; i < orderList.length; i++){
                                        //如果是由自取订单改为配送，则需要播放不同的音频
                                        if(!orderList[i].isChangeToDelivery){
                                            that.speckText();
                                        }else{
                                            that.speckTextChangeToDelivery();
                                        }
                    ...
                    batchUpdateIsPrintedTrue(idList) { // 获取订单列表
                        ...
                        $.ajax({
                            type: "post",
                            url: urlPrefix + '/rest/admin/order/batchUpdateIsPrintedTrue',
                            async: false,
                            data: {"idListStr" : JSON.stringify(idList)},
                            beforeSend: function(request) {
                                request.setRequestHeader("token", sessionStorage.getItem("token"));
                            },                                 
                            success: function(data){
                    ...
                    printOrderDetail: function (id) {
                        ...
                        $.ajax({
                            type: "post",
                            url: urlPrefix + '/rest/admin/order/selectById',
                            async: false,
                            data: {"id" : id},
                            beforeSend: function(request) {
                                request.setRequestHeader("token", sessionStorage.getItem("token"));
                            ...
                            success: function(data){
                                ...
                                $.ajax({
                                    type: "post",
                                    url: urlPrefix + '/rest/admin/orderDetail/list',
                                    async: false,
