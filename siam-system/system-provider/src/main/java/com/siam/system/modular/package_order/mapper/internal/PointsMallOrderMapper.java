...
public interface PointsMallOrderMapper extends BaseMapper<PointsMallOrder> {
    ...
    @Select("<script>select o.* from tb_mall_order o" +
            "<where> 1=1 " +
    ...
    @Select("<script>select o.*, " +
            /*"(SELECT if(COUNT(*)>0, false, true) FROM tb_mall_appraise WHERE order_id = o.id AND member_id = #{order.memberId}) as isAllowAppraise, " +*/
    ...
    @ResultMap("BaseResultMap")
    @Select("select o.* from tb_mall_order o where o.order_no = #{orderNo}")
    PointsMallOrderResult selectByPointsMallOrderNo(@Param("orderNo") String orderNo);

    //查询超时未支付订单
    @ResultMap("BaseResultMap")
    @Select("select o.* from tb_mall_order o where o.status = 1 and payment_deadline < now()")
    List<PointsMallOrderResult> selectOverduePointsMallOrder();

    @Update("<script>update tb_mall_order o set is_printed = 1 " +
    ...
    @Select("select max(queue_no) from tb_mall_order where to_days(create_time) = to_days(now())")
    ...
    @Update("update tb_mall_order set order_completion_time=now(), update_time=now(), status=#{status} where DATEDIFF(now(), payment_success_time) >= 7 and status in(5)")
    ...
    @ResultMap("BaseResultMap")
    @Select("select o.* from tb_mall_order o where o.change_to_delivery_out_trade_no = #{changeToDeliveryOutTradeNo}")
    ...
    @Select("select count(*) as latelyMonthlySales from tb_mall_order as o "+
    ...
    @Select("<script>select IFNULL(sum(o.merchant_income), 0) from tb_mall_order as o "+
    ...
    @Select("<script>select IFNULL(sum(o.actual_price), 0) from tb_mall_order as o "+
            "<where> 1=1 and STATUS in (6, 7, 9) and (o.create_time between #{startTime} and #{endTime})" +
            "</where>"+
            "</script>")
    ...
    @Select("<script>select count(*) from tb_mall_order as o "+
            "<where> 1=1 and STATUS in (6, 7, 9) and (o.create_time between #{startTime} and #{endTime})" +
            "<if test=\"order.memberId != null\"> AND o.member_id = #{order.memberId} </if>" +
            "</where>"+
            "</script>")
    ...
    @Select("SELECT if(o.status in (4), true, false) FROM tb_mall_order as o WHERE o.id = #{order.id}")
    boolean getIsAllowApplyRefund(@Param("order") PointsMallOrderParam param);

    @Select("SELECT if(o.status not in (1, 4, 7, 8, 9, 10, 11) and TIMESTAMPDIFF(HOUR, o.payment_success_time, now()) < 20*24, true, false) FROM tb_mall_order as o WHERE o.id = #{order.id}")
    boolean getIsAllowApplyAfterSales(@Param("order") PointsMallOrderParam param);

    @Select("SELECT if(o.status not in (1, 7, 8, 9, 10) and TIMESTAMPDIFF(HOUR, o.payment_success_time, now()) < 20*24, true, false) FROM tb_mall_order as o WHERE o.id = #{order.id}")
    boolean getIsAllowApplyAfterSalesByAdmin(@Param("order") PointsMallOrderParam param);

    @Select("SELECT if(o.status in (5, 6), true, false) FROM tb_mall_order as o WHERE o.id = #{order.id}")
    boolean getIsShowLogistics(@Param("order") PointsMallOrderParam param);

    @ResultMap("CustomResultMap")
    @Select("<script>select o.*, or1.type as refundType, or1.refund_way as refundWay, or1.refund_reason as refundReason, or1.refund_amount as refundAmount, or1.status as refundStatus, or1.create_time as refundCreateTime from tb_mall_order o " +
            "left join tb_mall_order_refund or1 on or1.order_id = o.id " +
    ...
    @ResultMap("BaseResultMap")
    @Select("SELECT o.* FROM tb_mall_order o where o.status in (6, 9) and TIMESTAMPDIFF(HOUR, o.payment_success_time, now()) > 24 and o.is_pay_to_merchant = 0")
    List<PointsMallOrderResult> selectByNeedPayPointsMallOrderFrozenBalanceOfMerchant();

    @Select("SELECT o.shop_id FROM tb_mall_order o WHERE o.status IN (2, 4) AND TIMESTAMPDIFF(MINUTE, o.payment_success_time, NOW()) > 10 GROUP BY o.shop_id")
    List<Integer> selectShopIdByOvertimePointsMallOrder();

    @Select("<script>SELECT DATE_FORMAT(create_time, '%Y-%m-%d') AS orderDate, COUNT(*) AS orderCount, IFNULL(SUM(actual_price), 0) AS orderAmount, IFNULL(SUM(merchant_income), 0) AS merchantIncome FROM tb_mall_order " +
    ...
    @Select("<script>SELECT DATE_FORMAT(create_time, '%Y-%m-%d') AS orderDate FROM tb_mall_order " +
            "<where> STATUS = 6 " +
    ...
    //查询支付人数
    @Select("<script>select count(DISTINCT(o.member_id)) from tb_mall_order as o "+
            "<where> 1=1 and STATUS not in (1, 10) and (o.create_time between #{startTime} and #{endTime})" +
            "</where>"+
            "</script>")
    ...
    @ResultMap("BaseResultMap")
    @Select("SELECT o.* FROM tb_mall_order o where o.status in (5, 6, 7) and o.is_sign = 0")
