...
public interface PointsMallCouponsMapper extends BaseMapper<PointsMallCoupons> {
    ...
    @Select("<script>select c.* from tb_mall_coupons c" +
            "<where> 1=1 " +
    ...
    @Select("<script>SELECT c.*, " +
            "(SELECT COUNT(*) FROM tb_mall_coupons_member_relation WHERE coupons_id = c.id) as gaveCount, " +
            "(SELECT COUNT(*) FROM tb_mall_coupons_member_relation WHERE coupons_id = c.id AND is_used = 1) as usedCount " +
            "FROM tb_mall_coupons c " +
            "<where> 1=1 " +
            "<if test=\"coupons.id != null\"> AND c.id = #{coupons.id} </if>" +
            "<if test=\"coupons.name != null and coupons.name !=''\"> AND c.name like '%${coupons.name}%' </if>" +
            "<if test=\"coupons.preferentialType != null\"> AND c.preferential_type = #{coupons.preferentialType} </if>" +
    ...
    @Select("<script>SELECT c.*, " +
            ...
            "(SELECT COUNT(*) FROM tb_mall_coupons_member_relation WHERE coupons_id = c.id AND is_used = 1) as usedCount " +
            "FROM tb_mall_coupons c " +
            "LEFT JOIN tb_mall_coupons_shop_relation csr on csr.coupons_id = c.id" +
            "<where> 1=1 " +
