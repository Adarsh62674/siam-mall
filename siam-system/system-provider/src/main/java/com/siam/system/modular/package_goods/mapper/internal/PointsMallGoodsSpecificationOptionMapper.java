...
public interface PointsMallGoodsSpecificationOptionMapper extends BaseMapper<PointsMallGoodsSpecificationOption> {
    ...
    @Select("<script>select so.* from tb_mall_goods_specification_option so" +
            "<where> 1=1 " +
    ...
    @Select("<script>select so.*, s.name AS specificationName, g.name AS goodsName, g.main_image AS goodsMainImage from tb_mall_goods_specification_option so " +
            "LEFT JOIN tb_mall_goods_specification s ON s.id = so.goods_specification_id " +
            "LEFT JOIN tb_mall_goods g ON g.id = s.goods_id " +
            "<where> 1=1 " +
    ...
    @Select("select IFNULL(max(so.sort_number), 0) from tb_mall_goods_specification_option so where so.goods_specification_id = #{goodsSpecificationId}")
    int selectMaxSortNumberByPointsMallGoodsSpecificationId(@Param("goodsSpecificationId") Integer goodsSpecificationId);

    @Select("<script>select IFNULL(sum(so.price), 0) from tb_mall_goods_specification_option so" +
...
//    @Update("update tb_mall_goods_specification_option set price = #{goodsAccessories.price}, stock = #{goodsAccessories.stock}, update_time = now() where name = #{goodsAccessories.name}")
//    void updateByPointsMallGoodsAccessories(@Param("goodsAccessories") PointsMallGoodsAccessories goodsAccessories);

    @Delete("delete from tb_mall_goods_specification_option where goods_id = #{goodsId}")
