...
public interface PointsMallOrderRefundMapper extends BaseMapper<PointsMallOrderRefund> {
    ...
    @Select("<script>select or1.* from tb_mall_order_refund or1" +
            "<where> 1=1 " +
    ...
    @ResultMap("BaseResultMap")
    @Select("select or1.* from tb_mall_order_refund or1 where order_id = #{orderId} limit 1 ")
    PointsMallOrderRefund selectByPointsMallOrderId(Long orderId);

    @Select("<script>select IFNULL(sum(or.refund_amount), 0) as refundAmount from tb_mall_order_refund as `or` " +
