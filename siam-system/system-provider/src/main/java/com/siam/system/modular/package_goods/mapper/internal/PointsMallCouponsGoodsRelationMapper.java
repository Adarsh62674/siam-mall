...
public interface PointsMallCouponsGoodsRelationMapper extends BaseMapper<PointsMallCouponsGoodsRelation> {
    ...
    @Delete("delete from tb_mall_coupons_goods_relation where coupons_id = #{couponsId}")
    void deleteByPointsMallCouponsId(@Param("couponsId") int couponsId);

    @ResultMap("BaseResultMap")
    @Select("<script>select c.* from tb_mall_coupons_goods_relation c" +
            "<where> 1=1 " +
    ...
    @Select("select goods_id as goodsId from tb_mall_coupons_goods_relation where coupons_id=#{couponsId}")
    List<Integer> getGoodsIdByCouponsId(@Param("couponsId") Integer couponsId);

    @Delete("delete from tb_mall_coupons_goods_relation where goods_id = #{goodsId}")
