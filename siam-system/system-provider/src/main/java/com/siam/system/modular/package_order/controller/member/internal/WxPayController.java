...
import com.siam.package_common.util.StringUtilsPlus;
...
public class WxPayController {
    ...
    public BasicResult toPay4Applet(@RequestBody @Validated WxPayDto wxPayDto, HttpServletRequest request) {
        ...
        if(wxPayDto.getType() == Quantity.INT_1){
            ...
            if(dbOrder.getShoppingWay() != Order.SHOPPING_WAY_OF_PICKUP){
...
//            }
        ...
        try {
            //生成的随机字符串
            String nonce_str = StringUtilsPlus.getRandomStringByLength(32);
            //获取本机的ip地址
            String spbill_create_ip = IpUtils.getIpAddr(request);
            //订单描述
            String body = wxPayDto.getType()==1 ? "订单付款" : (wxPayDto.getType()==2 ? "会员充值" : (wxPayDto.getType()==3 ? "自取订单改为配送" : (wxPayDto.getType()==4 ? "商城订单付款" : "-")));
