...
import com.siam.system.modular.package_goods.model.example.internal.PointsMallCouponsMemberRelationExample;
...
public interface PointsMallCouponsMemberRelationMapper extends BaseMapper<PointsMallCouponsMemberRelation> {
    ...
    @Select("select count(id) from tb_mall_coupons_member_relation where coupons_id = #{couponsId}")
    Integer getCountByPointsMallCouponsId(@Param("couponsId") int couponsId);

    @ResultMap("CustomResultMap")
    @Select("<script>select cmr.*, c.preferential_type as preferentialType, c.discount_amount as discountAmount, c.limited_price as limitedPrice, c.reduced_price as reducedPrice, c.description, c.source from tb_mall_coupons_member_relation cmr " +
            "left join tb_mall_coupons c on cmr.coupons_id = c.id" +
            "<where> 1=1 " +
    ...
    @Update("update tb_mall_coupons_member_relation set is_expired=1,update_time=NOW() where end_time<=#{date} and is_expired=0")
    int updateOverduePointsMallCoupons(@Param("date") Date date);

    @Select("select count(id) from tb_mall_coupons_member_relation where member_id=#{memberId} and is_used=0 and is_expired=0 and is_valid=1 ")
    Integer getCountsByMemberId(@Param("memberId") Integer memberId);

    @Update("update tb_mall_coupons_member_relation set is_valid=0,update_time=NOW() where coupons_id=#{couponsId}")
    int updateValidToFalseByPointsMallCouponsId(@Param("couponsId") Integer couponsId);

    @Update("update tb_mall_coupons_member_relation set end_time=#{date},update_time=NOW() where coupons_id=#{couponsId}")
    int updateEndTime(@Param("date") Date date, @Param("couponsId") Integer couponsId);

    @Update("update tb_mall_coupons_member_relation set end_time=date_add(start_time,interval #{days} day),update_time=NOW() where coupons_id=#{couponsId}")
