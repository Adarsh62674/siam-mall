...
public interface PointsMallGoodsSpecificationMapper extends BaseMapper<PointsMallGoodsSpecification> {
    ...
    @Select("<script>select s.* from tb_mall_goods_specification s " +
            "<where> 1=1 " +
    ...
    @Select("<script>select s.*, g.name AS goodsName, g.main_image AS goodsMainImage from tb_mall_goods_specification s " +
    ...
    @Select("select IFNULL(max(s.sort_number), 0) from tb_mall_goods_specification s where s.goods_id = #{goodsId}")
    int selectMaxSortNumberByPointsMallGoodsId(@Param("goodsId") Integer goodsId);

    @Select("select s.* from tb_mall_goods_specification s where s.goods_id = #{goodsId} and s.name = #{name}")
    PointsMallGoodsSpecification selectByPointsMallGoodsIdAndName(@Param("goodsId") Integer goodsId, @Param("name") String name);

    @Delete("delete from tb_mall_goods_specification where goods_id = #{goodsId}")
