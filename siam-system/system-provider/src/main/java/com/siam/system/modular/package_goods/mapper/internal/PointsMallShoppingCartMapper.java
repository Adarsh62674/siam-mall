...
public interface PointsMallShoppingCartMapper extends BaseMapper<PointsMallShoppingCart> {
    ...
    @Select("<script>select sc.* from tb_mall_shopping_cart sc" +
            "<where> 1=1 " +
    ...
    @Select("<script>select sc.*, g.name as goodsName, g.main_image as mainImage, g.price as goodsPrice, g.sale_price as salePrice, g.stock, g.status as goodsStatus, g.is_sale as isSale, g.packing_charges as packingCharges from tb_mall_shopping_cart sc" +
    ...
    @Select("<script>select count(*) from tb_mall_shopping_cart sc" +
    ...
    @Delete("<script>delete from tb_mall_shopping_cart" +
    ...
    @Update("update tb_mall_shopping_cart set is_goods_exists = 0 where goods_id = #{goodsId}")
    void updateIsGoodsExistsTo0ByPointsMallGoodsId(@Param("goodsId") int goodsId);

    @Select("select IFNULL(sum(sc.number), 0) from tb_mall_shopping_cart as sc "+
