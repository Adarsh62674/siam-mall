...
public class AdminPointsMallGoodsSpecificationOptionController {
    ...
    public BasicResult insert(@RequestBody @Validated(value = {}) PointsMallGoodsSpecificationOptionDto goodsSpecificationOptionDto){
        ...
        if(dbSpecification == null){
            ...
            example.createCriteria().andPointsMallGoodsIdEqualTo(goodsSpecificationOptionDto.getGoodsId()).andPointsMallGoodsSpecificationIdEqualTo(dbSpecification.getId()).andNameEqualTo(goodsSpecificationOptionDto.getName());
            int result = goodsSpecificationOptionService.countByExample(example);
            if(result > 0){
                basicResult.setSuccess(false);
                basicResult.setCode(BasicResultCode.ERR);
                basicResult.setMessage("规格名称重复，添加失败");
                return basicResult;
            }
        }
    ...
    public BasicResult update(@RequestBody @Validated(value = {}) PointsMallGoodsSpecificationOptionDto goodsSpecificationOptionDto){
        ...
        if(dbPointsMallGoodsSpecificationOption == null){
            basicResult.setSuccess(false);
            basicResult.setCode(BasicResultCode.ERR);
            basicResult.setMessage("该规格不存在，修改失败");
            return basicResult;
        }

        //查询原规格名称
        PointsMallGoodsSpecification dbSpecification = goodsSpecificationService.selectByPrimaryKey(dbPointsMallGoodsSpecificationOption.getGoodsSpecificationId());
        //判断规格名称是否被修改 以及 新规格名称是否存在
        if(dbPointsMallGoodsSpecificationOption.equals(dbSpecification.getName())){
            //规格名称未被修改
            //判断大规格+小规格是否重复
            PointsMallGoodsSpecificationOptionExample example = new PointsMallGoodsSpecificationOptionExample();
            example.createCriteria().andPointsMallGoodsIdEqualTo(dbPointsMallGoodsSpecificationOption.getGoodsId()).andPointsMallGoodsSpecificationIdEqualTo(dbSpecification.getId()).andNameEqualTo(goodsSpecificationOptionDto.getName()).andIdNotEqualTo(goodsSpecificationOptionDto.getId());
            int result = goodsSpecificationOptionService.countByExample(example);
            if(result > 0){
                basicResult.setSuccess(false);
                basicResult.setCode(BasicResultCode.ERR);
                basicResult.setMessage("规格名称重复，修改失败");
                return basicResult;
            }
        }else{
            //规格名称被修改
            //判断新规格名称是否存在
            dbSpecification = goodsSpecificationService.selectByPointsMallGoodsIdAndName(dbPointsMallGoodsSpecificationOption.getGoodsId(), goodsSpecificationOptionDto.getSpecificationName());
            if(dbSpecification == null){
                //查询商品规格的最大排序号
                int maxSortNumber = goodsSpecificationService.selectMaxSortNumberByPointsMallGoodsId(goodsSpecificationOptionDto.getGoodsId());
                //先添加商品规格
    ...
    public BasicResult delete(@RequestBody @Validated(value = {}) PointsMallGoodsSpecificationOption param){
        ...
        for(int id : param.getIds()){
            ...
            if(dbPointsMallGoodsSpecificationOption == null){
                basicResult.setSuccess(false);
                basicResult.setCode(BasicResultCode.ERR);
                basicResult.setMessage("该规格不存在，删除失败");
                return basicResult;
            }
