...
public interface PointsMallOrderRefundProcessMapper extends BaseMapper<PointsMallOrderRefundProcess> {
    ...
    @Select("<script>select orp.* from tb_mall_order_refund_process orp" +
            "<where> 1=1 " +
    ...
    @ResultMap("BaseResultMap")
    @Select("select orp.* from tb_mall_order_refund_process orp where order_refund_id = #{orderRefundId} order by create_time desc")
