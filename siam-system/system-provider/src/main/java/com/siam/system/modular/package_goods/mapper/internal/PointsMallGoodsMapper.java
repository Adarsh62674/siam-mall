...
public interface PointsMallGoodsMapper extends BaseMapper<PointsMallGoods> {
    ...
    @Select("<script>select g.* from tb_mall_goods g" +
            "<where> 1=1 " +
    ...
    @Select("<script>select g.*, " +
            "(select ifnull(sum(od.number), 0) from tb_mall_order_detail as od left join tb_mall_order as o on od.order_id = o.id where od.goods_id = g.id and o.status = 6) as totalSales, " +
            "m.id AS menuId, m.name AS menuName FROM tb_mall_goods g " +
            "left join tb_mall_menu_goods_relation mgr ON g.id = mgr.goods_id " +
            "left join tb_mall_menu m ON m.id = mgr.menu_id " +
            "<where> 1=1 " +
    ...
    @ResultMap("CustomResultMap")
    @Select("<script>select g.*, " +
            "(select ifnull(sum(od.number), 0) from tb_mall_order_detail as od left join tb_mall_order as o on od.order_id = o.id where od.goods_id = g.id and o.status = 6 and (o.create_time between #{goodsPointsMallMenuDto.startTime} and #{goodsPointsMallMenuDto.endTime})) as latelyMonthlySales, " +
            "m.id AS menuId, m.name AS menuName FROM tb_mall_goods g " +
            "left join tb_mall_menu_goods_relation mgr ON g.id = mgr.goods_id " +
            "left join tb_mall_menu m ON m.id = mgr.menu_id " +
            "<where> 1=1 " +
    ...
    @ResultMap("BaseResultMap")
    @Select("select g.* from tb_mall_goods as g,tb_mall_coupons_goods_relation as cgr where g.id=cgr.goods_id and cgr.coupons_id=#{couponsId}")
    List<PointsMallGoods> getListByPointsMallCouponsId(Integer couponsId);

    @ResultMap("BaseResultMap")
    @Select("select * from tb_mall_goods where status = 2 order by total_sales desc, create_time desc limit 3")
    List<PointsMallGoods> getListByTotalSalesTop3();

    @ResultMap("CustomResultMap")
    @Select("select g.*, " +
            "(select ifnull(sum(od.number), 0) from tb_mall_order_detail as od left join tb_mall_order as o on od.order_id = o.id where od.goods_id = g.id and o.status = 6 and (o.create_time between #{startTime} and #{endTime})) as latelyMonthlySales " +
            "from tb_mall_goods as g " +
            "where g.status = 2 " +
    ...
    @Update("update tb_mall_goods set monthly_sales=monthly_sales+#{num} where id=#{goodsId}")
    int updateMonthlySales(@Param("goodsId") Integer goodsId, @Param("num") Integer num);

    @Update("update tb_mall_goods set total_sales=total_sales+#{num} where id=#{goodsId}")
    int updateTotalSales(@Param("goodsId") Integer goodsId, @Param("num") Integer num);

    @Update("update tb_mall_goods set monthly_sales=0 ")
    int monthlySalesReset();

    @ResultMap("BaseResultMap")
    @Select("select * from tb_mall_goods where status = 2 order by create_time desc limit 1")
    List<PointsMallGoods> getListByLastestShelvesTop1();

    @ResultMap("CustomResultMap")
    @Select("<script>select g.*, " +
            "(select ifnull(sum(od.number), 0) from tb_mall_order_detail as od left join tb_mall_order as o on od.order_id = o.id where od.goods_id = g.id and o.status = 6 and (o.create_time between #{startTime} and #{endTime})) as latelyMonthlySales " +
            "from tb_mall_goods g" +
