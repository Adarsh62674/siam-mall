...
//    private WxPublicPlatformSubscribeUserService wxPublicPlatformSubscribeUserService;
    ...
    @Autowired
    private PointsMallCouponsService mallCouponsService;
...
//    }
    ...
    public Member findMemberByOneKeyLogin(Member member) throws Exception {
        //获取手机号码
       if(!StringUtilsPlus.isEmpty(member.getOpenId())&&!StringUtilsPlus.isEmpty(member.getNickname())&&!StringUtilsPlus.isEmpty(member.getMobile())){
...
//        }
    ...
    private Member autoRegister(MemberParam param, int registerWay){
        ...
        PointsMallCoupons dbMallCoupons = mallCouponsService.selectByPrimaryKey(BusinessType.NEW_PEOPLE_COUPONS_ID);
        if (dbMallCoupons != null) {
            ...
            mallCouponsMemberRelationService.insertSelective(couponsMemberRelation);
        }else{
            log.error("商城-系统默认优惠券-新人3折卷不存在");
        }
        ...
        if (param.getInviterId() != null && !param.getInviterId().equals("") && !param.getInviterId().equals("undefined")) {
            ...
            if (inviter != null && inviter.getId() != null) {
                ...
                PointsMallCoupons mallInviteCoupons = mallCouponsService.selectByPrimaryKey(BusinessType.INVITE_NEW_PEOPLE_COUPONS_ID);
                if (mallInviteCoupons != null) {
                    PointsMallCouponsMemberRelation inviteCouponsMemberRelation = new PointsMallCouponsMemberRelation();
                    ...
                    mallCouponsMemberRelationService.insertSelective(inviteCouponsMemberRelation);
                }else{
                    log.error("商城-系统默认优惠券-推荐新人3折卷不存在");
                }
    ...
    public MemberResult getLoginMemberInfo(MemberParam param) {
        ...
        //优惠券数量
        Integer mallCountsCouponsNumber = mallCouponsMemberRelationService.getCountsByMemberId(loginMember.getId());
        Integer couponsNumber = mallCountsCouponsNumber;
        ...
        memberResult.setCouponsNumber(couponsNumber);
        return memberResult;
    }
