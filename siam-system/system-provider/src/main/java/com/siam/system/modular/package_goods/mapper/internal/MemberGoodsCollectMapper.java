...
public interface MemberGoodsCollectMapper extends BaseMapper<MemberGoodsCollect> {
    ...
    @Update("update tb_member_goods_collect mgc set mgc.is_buy = (select if(count(0) > 0, 1, 0) from tb_mall_order_detail od where od.goods_id = mgc.goods_id) where mgc.is_buy = 0")
    void updateIsBuy();
}