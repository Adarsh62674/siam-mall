...
public interface PointsMallMemberGoodsCollectMapper extends BaseMapper<PointsMallMemberGoodsCollect> {
    ...
    @Select("<script>select sc.* from tb_mall_member_goods_collect sc" +
            "<where> 1=1 " +
    ...
    @Select("<script>select sc.*, g.name as goodsName, g.main_image as mainImage, g.price as goodsPrice, g.sale_price as salePrice, g.stock, g.status as goodsStatus, g.is_sale as isSale, g.packing_charges as packingCharges from tb_mall_member_goods_collect sc" +
            " left join tb_mall_goods g on g.id = sc.goods_id" +
            "<where> 1=1 " +
            "<if test=\"PointsMallMemberGoodsCollect.id != null\"> AND sc.id = #{PointsMallMemberGoodsCollect.id} </if>" +
            "<if test=\"PointsMallMemberGoodsCollect.memberId != null\"> AND sc.member_id = #{PointsMallMemberGoodsCollect.memberId} </if>" +
            "<if test=\"PointsMallMemberGoodsCollect.goodsId != null\"> AND sc.goods_id = #{PointsMallMemberGoodsCollect.goodsId} </if>" +
    ...
    @Select("<script>select count(*) from tb_mall_member_goods_collect sc" +
    ...
    @Delete("<script>delete from tb_mall_member_goods_collect" +
    ...
    @Update("update tb_mall_member_goods_collect set is_goods_exists = 0 where goods_id = #{goodsId}")
    void updateIsGoodsExistsTo0ByPointsMallGoodsId(@Param("goodsId") int goodsId);

    @Select("select IFNULL(sum(sc.number), 0) from tb_mall_member_goods_collect as sc "+
    ...
    @Update("update tb_mall_member_goods_collect mgc set mgc.is_buy = (select if(count(0) > 0, 1, 0) from tb_mall_order_detail od where od.goods_id = mgc.goods_id) where mgc.is_buy = 0")
    void updateIsBuy();
}